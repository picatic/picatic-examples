...
checkout = (data, payload, userid) => {
  ...
  const eventid = value.split('-')[0]
  const ticketid = value.split('-')[1]
  const realname = data.real_name
  const index = realname.indexOf(' ')
  const firstname = realname.substr(0, index)
  const lastname = realname.substr(index + 1)
  const body = {
    data: {
      attributes: {
  ...
  unirest
    .post('https://picatic.p.mashape.com/checkout')
...
confirm = (checkoutid, userid) => {
  //Copy & Paste from Rapid Api Picatic integration code section: https://rapidapi.com/user/thomas_mirmo/package/Picatic%20Ticketing
  unirest
    .post('https://picatic.p.mashape.com/checkout/' + checkoutid + '/confirm')
    .header('X-Mashape-Key', 'Your-Mashape-key')
    .header('X-Mashape-Host', 'picatic.p.mashape.com')
    .header('Content-Type', 'application/x-www-form-urlencoded')
    .send('')
    .end(function(result) {
      const orderCreatedAt = moment
        .utc(result.body.data.attributes.created)
        .format('YYYY-MM-DD HH:mm:ss')
      const orderToken = CryptoJS.SHA1(
        result.body.data.attributes.invoice.invoice_id + orderCreatedAt,
      ).toString()
      ...
      rapid
        .call('Slack', 'postMessage', {
          ...
          channel: userid,
          text:
            '<https://www.picatic.com/tickets/html/invoice:' +
            result.body.data.attributes.invoice.invoice_id +
            '/token:' +
            orderToken +
            '|Click here to view your ticket!>   OR    <https://api.picatic.com/v2/checkout/' +
            result.body.data.id +
            '/ticket/' +
            result.body.data.attributes.tickets[0].ticket_id +
            '/render/pkpass|Add to Apple Wallet>',
          unfurlLinks: 'true',
